...
const yScaleChange = d3.scaleOrdinal().domain([0, 1]).range([height * 2 / 4, height * 3 / 4 + 50])
...
const rectWidth = windowWidth > 800 ? 24 : 18;
const radius = windowWidth > 800 ? 16 : 10;
...
.attr("class", "text")
.attr("fill", "black")
.attr("font-size", windowWidth > 1100 ? 12 : 10)
...
svg.append("text").attr("class", "mainText").attr("x", 50).attr("y", 100).text("How the UN member states voted on General Assembly Resolutions related to Ukraine and Gaza (Oct / Dec)").attr("fill", "white").call(wrap, windowWidth > 1200 ? windowWidth / 3 : 300)
...
if (index == 1) {
    ...
    d3.selectAll(".Gaza_Dec").transition().duration(100).style("opacity", 0)
    d3.selectAll(".Gaza_Oct").transition().duration(100).style("opacity", 0)
    setTimeout(function() {
        d3.selectAll(".Ukraine").transition().duration(100).attr("width", rectWidth);
    }, 600)
}

if (index === 2) {
    d3.select(".mainText").text("Vote distribution, out of a total of 193:").call(wrap, windowWidth > 1200 ? windowWidth / 3 : 300)
    d3.selectAll(".annotations").style("opacity", 1).call(wrap, windowWidth < 500 ? 50 : 100)
    d3
...
if (index === 3) {
    ...
    svg.selectAll("text.annotations").data(voteRegion).join("text").attr("class", "annotations").style("opacity", 1).attr("text-anchor", "middle").attr("x", d => xScaleRegions(d.category)).attr("y", d => height / 4).text(d => `${d.category} (${d.count})`).style("opacity", 1).attr("fill", "white").call(wrap, windowWidth < 500 ? 50 : 100)
...
if (index === 4) {
    ...
    d3.selectAll(".Ukraine").transition().duration(100).style("opacity", 0)
    d3.selectAll(".Gaza_Oct").transition().duration(100).attr("width", rectWidth / 2).attr("transform", `translate(${-rectWidth/3},0)`) // was at 8, move to 0
    d3.selectAll(".Gaza_Dec").transition().duration(100).attr("width", rectWidth / 2).attr("transform", `translate(${-rectWidth/6},0)`) // wa
...
if (index === 5) {
    ...
    d3.selectAll("text.annotations").data(voteNumGazaOct).join("text").attr("class", "annotations").attr("x", d => xScale(d.category) - 40).attr("y", height / 4).text(d => `${d.category.replaceAll("_", " ")}: ${d.count} (${(d.count/193*100).toFixed(1)}%)`).style("opacity", 1).call(wrap, windowWidth < 600 ? 40 : 100)
...
if (index === 6) {
    ...
    d3.selectAll("text.annotations").data(voteNumGazaDec).join("text").attr("class", "annotations").attr("x", d => xScale(d.category) - 40).attr("y", height / 4).text(d => `${d.category.replaceAll("_", " ")}: ${d.count} (${(d.count/193*100).toFixed(1)}%)`).style("opacity", 1).call(wrap, windowWidth < 600 ? 40 : 100)
...
if (index === 8) {
    ...
    svg.selectAll("text.annotations").data(voteRegion).join("text").attr("class", "annotations").style("opacity", 1).attr("text-anchor", "middle").attr("x", d => xScaleRegions(d.category)).attr("y", d => height / 4).text(d => `${d.category} (${d.count})`).style("opacity", 1).attr("fill", "white").call(wrap, windowWidth < 600 ? 40 : 100)
    d3
        .forceSimulation(nodes)
        .force(
            "x",
...
if (index === 9) {
    d3.select("#next").style("pointer-events", "none").style("cursor", "none").style("opacity", 1).html("...you've reached the end")
    $('#next').off('click');
    ...
    svg.selectAll("text.annotations").data(votesAgainst).join("text").attr("class", "annotations").style("opacity", 1).attr("text-anchor", "middle").attr("x", d => xScaleAgainst(d)).attr("y", d => height / 4).text(d => d).style("opacity", 1).attr("fill", "white").call(wrap, windowWidth < 600 ? 40 : 100)
